# Lab 2: Teju Edidi

# variable = container for value
name = "Teju Edidi" # 1

nickname = "Sunny" # 2

# int or integer = whole number (no decimal) -1 10 500
# float or floating point (decimal) 3.2 -4.5
age = 17 # 3
print("Sunny's age is", age)

used_before = True

hobbies = ["painting", "reading", "coding"]

# dict_name = {key:value, key:value, key:value}

food = {"food": "sushi"}























# Step 1: Name Variable
name = "Tejaswini Edidi"

# Step 2: Nickname Variable
nickname = "Teju" 

# Step 3: Age Variable
# numeric data type - int or integer (whole number)
# float or floating point (decimal)
age = 23

# Step 4: Boolean Variable
# True or False
used_before = True

# Step 5: List Data Structure 
# [ ]
hobbies = ["painting", "cat sitting", "craft", "TV"] # list of strings

# Step 6: Dictionary Data Structure
# (key : value) pairs 
# { }
my_favs = {"cartoon": "steven universe", "food": ["sushi", "pizza", "burgers", "fries"], "flower":"sunflower"}

# Step 7: Set Data Structure
# collection of items - unique (no duplicates) unordered
teachers = {"sydney", "sydney", "parker", "cris", "teju"}
print(teachers)

# Step 8: Tuple Data Structure
# collection of items - unchangeable ordered can have duplicates ( )
teachers1 = ("sydney",  "parker", "cris", "sydney", "teju")
print(teachers1)
